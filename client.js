...
`🎁 Промокод <code>${promo.code}</code>\n\nПрименений: ${promo.limit||'∞'}\nОсталось: ${remaining}\nДействует на: ${promo.category}\nСрок до: ${promo.expires||'Бессрочно'}\nКомментарий: "${promo.comment||'Нет'}"`,
        ...
        if (state.filterStatus) {
            if (text === 'Назад') { temp[chatId] = {}; return bot.sendMessage(chatId, 'Выберите фильтр:', { reply_markup: { inline_keyboard: [
                [{ text: 'По статусу заказа', callback_data: 'filter_status' }],
                [{ text: 'По никнейму',        callback_data: 'filter_nickname' }],
                [{ text: 'По цене',            callback_data: 'filter_price' }],
                [{ text: 'По трек номеру',     callback_data: 'filter_track' }],
            ...
            temp[chatId] = {};
            if (!filtered.length) return bot.sendMessage(chatId, 'Заказов с таким статусом нет');
        ...
        if (state.filterPrice) {
            ...
            if (!filtered.length) return bot.sendMessage(chatId, 'В этом диапазоне заказов нет', { reply_markup: { keyboard: [
                ['0-5000'],['5000-10000'],['10000-15000'],['15000-20000'],['20000+'],['Назад']
            ], resize_keyboard: true }});
            temp[chatId] = {};
            return sendOrdersWithButtons(chatId, filtered);
        }
...
bot.on('callback_query', async query => {
    ...
    if (data === 'filter_status') { temp[chatId] = { filterStatus: true }; return bot.sendMessage(chatId, 'Выберите статус:', { reply_markup: { keyboard: [
        ['В обработке'],['Отправлен на склад в Китае'],['Отправлен на склад в Сочи'],['Готов к получению'],['Завершен'],['Назад']
    ], resize_keyboard: true }}); }
    ...
    if (data === 'filter_price') { temp[chatId] = { filterPrice: true }; return bot.sendMessage(chatId, 'Выберите диапазон:', { reply_markup: { keyboard: [
        ['0-5000'],['5000-10000'],['10000-15000'],['15000-20000'],['20000+'],['Назад']
    ], resize_keyboard: true }}); }
